...
When("accedo al módulo Monitorización y selecciono la entidad {string}", (entidad) => {
  // Navegar al módulo Monitorización y seleccionar la entidad
  cy.visit("/monitorizacion"); // Ruta del módulo Monitorización
...
Then("debería poder crear un nuevo registro correctamente", () => {
  ...
  cy.get("input#campo-nombre").type("Nuevo Registro Monitorización"); // Ingresar un nombre para el nuevo registro
...
Then("debería poder actualizar un registro correctamente", () => {
  ...
  cy.get("input#campo-nombre").clear().type("Registro Actualizado Monitorización"); // Modificar el nombre
