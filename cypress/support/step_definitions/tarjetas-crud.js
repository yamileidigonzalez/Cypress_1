...
When("accedo al módulo Tarjetas y selecciono la entidad {string}", (entidad) => {
  // Navegar al módulo Tarjetas y seleccionar la entidad
  cy.visit("/tarjetas"); // Ruta del módulo Tarjetas
...
Then("debería poder crear un nuevo registro correctamente", () => {
  ...
  cy.get("input#campo-nombre").type("Nuevo Registro Tarjetas"); // Ingresar un nombre para el nuevo registro
...
Then("debería poder actualizar un registro correctamente", () => {
  ...
  cy.get("input#campo-nombre").clear().type("Registro Actualizado Tarjetas"); // Modificar el nombre
